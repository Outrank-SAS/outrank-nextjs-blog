...
const Pagination = ({ basePath, currentPage, totalPages }: Props) => {
  ...
  return (
    <nav className="mt-14 flex flex-wrap items-center justify-start gap-1.5" aria-label="Pagination">
      {showArrows && currentPage > 1 ? (
        <Link
          href={getPageHref(basePath, currentPage - 1)}
          aria-label="Previous page"
          className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-md border border-slate-200 bg-white text-slate-700 transition hover:border-slate-300 hover:bg-slate-50 hover:text-slate-950"
        >
          <svg
            width="16"
            height="16"
