...
const EmptySearchIllustration = () => (
  ...
  >
    {/* Background blob */}
    <circle cx="210" cy="160" r="110" fill="rgb(var(--signal-accent) / 0.1)" />
    ...
    <g transform="rotate(-8 110 90)">
      ...
      <rect x="52" y="62" width="40" height="40" rx="4" fill="rgb(var(--signal-accent) / 0.2)" />
      <line x1="100" y1="68" x2="168" y2="68" stroke="rgb(var(--signal-accent))" strokeWidth="2.5" strokeLinecap="round" />
    ...
    <g transform="rotate(4 200 230)">
      ...
      <rect x="132" y="202" width="32" height="32" rx="4" fill="rgb(var(--signal-accent) / 0.2)" />
      <line x1="172" y1="208" x2="248" y2="208" stroke="rgb(var(--signal-accent))" strokeWidth="2.5" strokeLinecap="round" />
    ...
    {/* Magnifying glass */}
    <g>
      <circle cx="240" cy="130" r="52" fill="none" stroke="rgb(var(--signal-accent))" strokeWidth="3" />
      <circle cx="240" cy="130" r="47" fill="rgb(var(--signal-accent) / 0.08)" />
      <line
        x1="278"
        y1="168"
        x2="312"
        y2="202"
        stroke="rgb(var(--signal-accent))"
        strokeWidth="8"
        strokeLinecap="round"
      />
      <line x1="316" y1="206" x2="320" y2="210" stroke="rgb(var(--signal-accent))" strokeWidth="8" strokeLinecap="round" />
    </g>

    {/* Sparkles — 4-point stars */}
    <g fill="rgb(var(--signal-accent))">
    ...
    {/* Outlined dots */}
    <g fill="none" stroke="rgb(var(--signal-accent) / 0.5)" strokeWidth="1.5">
...
const EmptySearchState = ({ query, allArticles }: Props) => {
  ...
  return (
    <section className="rounded-lg border border-white/10 bg-white/[0.04] p-6 shadow-2xl shadow-black/30 md:p-12">
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-[1fr_1.1fr] lg:items-center lg:gap-12">
        ...
        <div className="flex flex-col items-center text-center lg:items-start lg:text-left">
          <h3 className="break-words text-2xl font-black leading-tight tracking-tight text-white md:text-4xl">
            No articles found for <span className="text-signal-accent">“{truncateQuery(query)}”</span>
          </h3>
          ...
          <div className="mt-7">
            <Link
              href="/blog"
              className="inline-flex h-11 items-center justify-center rounded-full bg-signal-accent px-6 text-sm font-bold text-zinc-950 transition hover:bg-signal-accent/90"
            >
              Browse all articles
            </Link>
          </div>
          {popularTags.length > 0 ? (
            <div className="mt-8 w-full border-t border-white/10 pt-6">
              <p className="text-xs font-medium text-zinc-400">Try searching for:</p>
              <div className="mt-3 flex flex-wrap justify-center gap-2 lg:justify-start">
                {popularTags.map((tag) => (
                  <Link
                    key={tag}
                    href={`/blog/tag/${encodeURIComponent(tag)}`}
                    className="inline-flex items-center rounded-full border border-white/15 bg-white/[0.04] px-3 py-1 text-xs font-semibold text-zinc-300 transition hover:border-white/30 hover:text-white"
                  >
                    {titleCaseTag(tag)}
                  </Link>
                ))}
