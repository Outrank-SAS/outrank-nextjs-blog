...
const BlogList = ({ paginatedArticles, allArticles, currentPage, totalPages }: Props) => {
  ...
  if (isSearching) {
    if (filteredResults.length === 0) {
      return (
        <div className="flex flex-col items-center bg-slate-50 px-6 py-16 text-center">
          <div className="flex h-16 w-16 items-center justify-center rounded-full bg-white text-slate-500">
            <FrownIcon />
          </div>
          <h3 className="mt-5 font-serif text-2xl font-black tracking-tight text-slate-950">
            No matches for <span className="text-studio-accent">“{trimmedQuery}”</span>
          </h3>
          ...
          <Link
            href="/blog"
            className="mt-7 inline-flex items-center rounded-md bg-studio-accent px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-studio-accent/90"
          >
            Show all articles
          </Link>
        </div>
